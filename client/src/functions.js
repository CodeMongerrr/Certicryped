...
// const variable_1 = useSelector((store) => (store.variables.variable_1));
let account = [];
...
export const loadContract = async () => {
    ...
    const certi = new web3.eth.Contract(
        certificate_abi,
        certificate_address
    );
    certificate = certi;
...
export const getNFTs = async (grantee) => {
    ...
    if (tokens !== null) { // Add a null check here
        const len = tokens.length;
        for (let i = 0; i < len; i++) { // Change <= to <
            let metadata = await certificate.methods.tokenURI(tokens[i]).call({ from: account });
            console.log("rhail");
            console.log(metadata);
